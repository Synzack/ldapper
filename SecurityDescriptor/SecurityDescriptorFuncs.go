...
func ParseSD(SD string, baseDN string, conn *ldap.Conn) (abusableAcesResult []CHECK_ABUSABLE_ACES) {
	header := getHeader(SD)
	...
	abusableAces := []CHECK_ABUSABLE_ACES{}
	for ace, _ := range rawACESList {
		entry := CHECK_ABUSABLE_ACES{SamAccountName: "", GENERIC_ALL: false, GENERIC_WRITE: false, WRITE_OWNER: false, WRITE_DACL: false, FORCE_CHANGE_PASSWORD: false, ADD_MEMBER: false}
	...
	return abusableAces
	/*fmt.Println("GENERIC_ALL:")
	for _, entry := range abusableAces {
		if entry.GENERIC_ALL == true {
			fmt.Println("\t" + entry.SamAccountName)
		}
	}
	...
	for _, entry := range abusableAces {
		if entry.FORCE_CHANGE_PASSWORD == true || entry.GENERIC_ALL == true {
			fmt.Println("\t" + entry.SamAccountName)
		}
	}
